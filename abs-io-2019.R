...
# from ABS Input-Output tables for 2018-2019
#
#############
...
# abs-io-2019-detailed.csv contains data from Table 5 of the Input-Output Tables of
# the ABS National Accounts
# TABLE 5: INDUSTRY BY INDUSTRY FLOW TABLE (DIRECT ALLOCATION OF IMPORTS)
table1 <- read.csv("data/abs-io-2019-detailed.csv", header = TRUE, row.names = 1)
...
for (i in 1:numIndustries) {
  A_Emp[,i] <- A[,1] * employment_coefficients
  emp_first_round_effects[i] <- sum(A[, i] * employment_coefficients)
}

A_Leontief_Inverse_Emp <- A_Leontief_Inverse
emp_simple_multipliers <- numeric(numIndustries)
for (i in 1:numIndustries) {
  A_Leontief_Inverse_Emp[,i] <- A_Leontief_Inverse[,1] * employment_coefficients
...
for (i in 1:numIndustries) {
  B_Star_Emp[,i] <- B_Star[,1] * employment_coefficients
...
write_csv(employment_multipliers,'data/io-employment-multipliers-2019.csv')
